 ...
 *
 * 10、dup/dup2/fcntl
 *         1) fd=open("somefile"...);               2) save_fd=dup(1);                                3) dup2(fd,1);
 *             +------+                                 +------+                                           +------+
 *         0   |      |--->tty                      0   |      |--->tty                                0   |      |--->tty
 *             +------+                                 +------+                                           +------+
 *         1   |      |--->tty                      1   |      |----------------->tty                  1   |      |---------+       tty
 ...
 *         2   |      |---->tty                     2   |      |---->tty          |                    2   |      |---->tty |        |
 *             +------+                                 +------+                  |                        +------+         ↓        |
 *       fd=3  |      |---->somefile              fd=3  |      |---->somefile     |                  fd=3  |      |---->somefile     |
 *             +------+                                 +------+                  |                        +------+                  |
 *             |      |                       save_fd=4 |      |------------------+              save_fd=4 |      |------------------+
 *             +------+                                 +------+                                           +------+                                 
 * 
 *         4) close(fd);write(1....);                 5) dup2(save_fd,1); write(1...);                 6) close(save_fd)
 *             +------+                                   +------+                                         +------+             
 *         0   |      |--->tty                        0   |      |--->tty                              0   |      |--->tty      
 *             +------+                                   +------+                                         +------+             
 *         1   |      |---------+       tty           1   |      |----------------------->tty          1   |      |--->tty      
 ...
 *         2   |      |---->tty |        |            2   |      |---->tty                 |           2   |      |---->tty
 *             +------+         ↓        |                +------+                         |               +------+             
 *       fd=3  |      |      somefile    |          fd=3  |      |   somefile(文件被close)  |         fd=3  |      |
 *             +------+                  |                +------+                         |               +------+             
 *   save_fd=4 |      |------------------+      save_fd=4 |      |-------------------------+     save_fd=4 |      |             
 *             +------+                                   +------+                                         +------+             
 *
 *		dup函数
 *			函数描述：复制文件描述符
 ...
 *				oldfd：要复制的文件描述符
 *      	返回值
 *      		成功：返回最小且没有被占用的文件描述符 ，此时旧的描述符和新的描述符都指向同一个文件，内核中会维持一个计数，这个计数类似于硬链接
 *      	    	 之后，必须要close新旧两个文件描述符（计数归0），才能保证把当前文件关闭。
 *      	     	验证方法：通过oldfd写操作，使用newfd对文件读操作。
 *          	失败：返回-1，设置errno值
 *
 *		dup2函数
 *			函数描述：复制文件描述符
...
/**
 * 使用dup2进行文件描述符复制，并对新旧两个描述符进行读写操作
 */
void dup2File(const char *path1 ,const char *path2);

/**
 * 文件描述符复制，并对新旧两个描述符进行读写操作
 */
