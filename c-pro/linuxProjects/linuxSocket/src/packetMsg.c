...
void packetMsgServer() {
	...
	if (acceptfd < 0) {
		close(socketfd);
		printf(
				"[createServerSocket]accept socket failed:socketfd is %d ,clientAddrLen is %d,acceptfd is %d\n",
	...
	printf(
			"[packetMsgServer]accept a client ,socketfd is %d , acceptfd is %d,clientIp is %s,clientPort is %d\n",
...
void packetMsgClient() {
	...
	for (int i = 0; i < 10; i++) {
		...
		memset(&timeStr, 0, sizeof(timeStr));
		toDateTimeCh(timeStr, 0);
		sprintf((char*) sendMsg.buf, "[%s]message from client , sendNum is %d ",
				timeStr, i);
		msgLen = strlen(sendMsg.buf);
		sendMsg.len = htonl(msgLen); //这里注意要统一字节序为网络字节序
