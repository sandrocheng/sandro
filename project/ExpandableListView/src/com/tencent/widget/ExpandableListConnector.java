...
/**
 * A {@link BaseAdapter} that provides data/Views in an expandable list (offers features such as collapsing/expanding
 * groups containing children). By itself, this adapter has no data and is a connector to a
 * {@link ExpandableListAdapter} which provides the data.
 * <p>
 * Internally, this connector translates the flat list position that the ListAdapter expects to/from group and child
 * positions that the ExpandableListAdapter expects.
 */
class ExpandableListConnector extends BaseAdapter implements Filterable
{
    ...
    /**
     * List of metadata for the currently expanded groups. The metadata consists of data essential for efficiently
     * translating between flat list positions and group/child positions. See {@link GroupMetadata}.
     */
    ...
    /**
     * Translates a flat list position to either a) group pos if the specified flat list position corresponds to a
     * group, or b) child pos if it corresponds to a child. Performs a binary search on the expanded groups list to find
     * the flat list pos if it is an exp group, otherwise finds where the flat list pos fits in between the exp groups.
     * 
     * @param flPos the flat list position to be translated
     * @return the group position or child position of the specified flat list position encompassed in a
     *         {@link PositionMetadata} object that contains additional useful info for insertion, etc.
     */
    PositionMetadata getUnflattenedPos(final int flPos)
    {
        ...
        {
            /*
             * There aren't any expanded groups (hence no visible children either), so flPos must be a group and its
             * group pos will be the same as its flPos
             */
            return PositionMetadata.obtain(flPos, ExpandableListPosition.GROUP, flPos, -1, null, 0);
        }

        /*
         * Binary search over the expanded groups to find either the exact expanded group (if we're looking for a group)
         * or the group that contains the child we're looking for. If we are looking for a collapsed group, we will not
         * have a direct match here, but we will find the expanded group just before the group we're searching for (so
         * then we can calculate the group position of the group we're searching for). If there isn't an expanded group
         * prior to the group being searched for, then the group being searched for's group position is the same as the
         * flat list position (since there are no children before it, and all groups before it are collapsed).
         */
        while (leftExpGroupIndex <= rightExpGroupIndex)
        {
            ...
            {
                /*
                 * The flat list position is after the current middle group's last child's flat list position, so search
                 * right
                 */
            ...
            {
                /*
                 * The flat list position is before the current middle group's flat list position, so search left
                 */
            ...
            {
                /*
                 * The flat list position is this middle group's flat list position, so we've found an exact hit
                 */
                return PositionMetadata.obtain(flPos, ExpandableListPosition.GROUP, midExpGm.gPos, -1, midExpGm,
                        midExpGroupIndex);
            }
            else if (flPos <= midExpGm.lastChildFlPos
            /*
             * && flPos > midGm.flPos as deduced from previous conditions
             */)
            {
                /* The flat list position is a child of the middle group */

                /*
                 * Subtract the first child's flat list position from the specified flat list pos to get the child's
                 * position within the group
                 */
                final int childPos = flPos - (midExpGm.flPos + 1);
                return PositionMetadata.obtain(flPos, ExpandableListPosition.CHILD, midExpGm.gPos, childPos, midExpGm,
                        midExpGroupIndex);
            }
        }

        /*
         * If we've reached here, it means the flat list position must be a group that is not expanded, since otherwise
         * we would have hit it in the above search.
         */

        /**
         * If we are to expand this group later, where would it go in the mExpGroupMetadataList ?
         */
        ...
        /*
         * To figure out exact insertion and prior group positions, we need to determine how we broke out of the binary
         * search. We backtrack to see this.
         */
        if (leftExpGroupIndex > midExpGroupIndex)
        {

            /*
             * This would occur in the first conditional, so the flat list insertion position is after the left group.
             * Also, the leftGroupPos is one more than it should be (since that broke out of our binary search), so we
             * decrement it.
             */
            ...
            /*
             * Sums the number of groups between the prior exp group and this one, and then adds it to the prior group's
             * group pos
             */
        ...
        else if (rightExpGroupIndex < midExpGroupIndex)
        {
            ...
            /*
             * This would occur in the second conditional, so the flat list insertion position is before the right
             * group. Also, the rightGroupPos is one less than it should be, so increment it.
             */
            ...
            /*
             * Subtracts this group's flat list pos from the group after's flat list position to find out how many
             * groups are in between the two groups. Then, subtracts that number from the group after's group pos to get
             * this group's pos.
             */
    ...
    /**
     * Translates either a group pos or a child pos (+ group it belongs to) to a flat list position. If searching for a
     * child and its group is not expanded, this will return null since the child isn't being shown in the ListView, and
     * hence it has no position.
     * 
     * @param pos a {@link ExpandableListPosition} representing either a group position or child position
     * @return the flat list position encompassed in a {@link PositionMetadata} object that contains additional useful
     *         info for insertion, etc., or null.
     */
    PositionMetadata getFlattenedPos(final ExpandableListPosition pos)
    {
        ...
        {
            /*
             * There aren't any expanded groups, so flPos must be a group and its flPos will be the same as its group
             * pos. The insert position is 0 (since the list is empty).
             */
        ...
        /*
         * Binary search over the expanded groups to find either the exact expanded group (if we're looking for a group)
         * or the group that contains the child we're looking for.
         */
        while (leftExpGroupIndex <= rightExpGroupIndex)
        {
            midExpGroupIndex = (rightExpGroupIndex - leftExpGroupIndex) / 2 + leftExpGroupIndex;
            ...
            else if (pos.groupPos == midExpGm.gPos)
            {
                ...
                if (pos.type == ExpandableListPosition.GROUP)
                {
                    ...
                    return PositionMetadata.obtain(midExpGm.flPos, pos.type, pos.groupPos, pos.childPos, midExpGm,
                            midExpGroupIndex);
                }
                else if (pos.type == ExpandableListPosition.CHILD)
                {
                    /* If it's a child, calculate the flat list pos */
                    return PositionMetadata.obtain(midExpGm.flPos + pos.childPos + 1, pos.type, pos.groupPos,
                            pos.childPos, midExpGm, midExpGroupIndex);
                }
                else
                {
                    return null;
        ...
        /*
         * If we've reached here, it means there was no match in the expanded groups, so it must be a collapsed group
         * that they're search for
         */
        if (pos.type != ExpandableListPosition.GROUP)
        {
        ...
        if (leftExpGroupIndex > midExpGroupIndex)
        {
             ...
             * This would occur in the first conditional, so the flat list insertion position is after the left group.
             * 
             * The leftGroupPos is one more than it should be (from the binary search loop) so we subtract 1 to get the
             * actual left group. Since the insertion point is AFTER the left group, we keep this +1 value as the
             * insertion point
             */
            final GroupMetadata leftExpGm = egml.get(leftExpGroupIndex - 1);
        ...
        else if (rightExpGroupIndex < midExpGroupIndex)
        {
            ...
            /*
             * This would occur in the second conditional, so the flat list insertion position is before the right
             * group. Also, the rightGroupPos is one less than it should be (from binary search loop), so we increment
             * to it.
             */
            final GroupMetadata rightExpGm = egml.get(++rightExpGroupIndex);
    ...
    {
        /*
         * Total count for the list view is the number groups plus the number of children from currently expanded groups
         * (a value we keep cached in this class)
         */
    ...
    public View getView(int flatListPos, View convertView, ViewGroup parent)
    {
        ...
        if (posMetadata.position.type == ExpandableListPosition.GROUP)
        {
            retValue = mExpandableListAdapter.getGroupView(posMetadata.position.groupPos, posMetadata.isExpanded(),
                    convertView, parent);
        }
        else if (posMetadata.position.type == ExpandableListPosition.CHILD)
        {
    ...
    public int getItemViewType(int flatListPos)
    {
        ...
        int retValue;
        if (VersionUtils.isrFroyo() && mExpandableListAdapter instanceof HeterogeneousExpandableList)
        {
     ...
     * 
     * @param forceChildrenCountRefresh Forces refreshing of the children count for all expanded groups.
     * @param syncGroupPositions Whether to search for the group positions based on the group IDs. This should only be
     *            needed when calling this from an onChanged callback.
     */
    ...
    private void refreshExpGroupMetadataList(boolean forceChildrenCountRefresh, boolean syncGroupPositions)
    {
        ...
        for (int i = 0; i < egmlSize; i++)
        {
            ...
            /*
             * Get the number of children, try to refrain from calling another class's method unless we have to (so do a
             * subtraction)
             */
            ...
            {
                /*
                 * Num children for this group is its last child's fl pos minus the group's fl pos
                 */
            ...
            /*
             * This skips the collapsed groups and increments the flat list position (for subsequent exp groups) by
             * accounting for the collapsed groups
             */
    ...
    {
        PositionMetadata pm = getFlattenedPos(ExpandableListPosition.obtain(ExpandableListPosition.GROUP, groupPos, -1,
                -1));
        if (pm == null)
            return false;
    ...
    boolean collapseGroup(PositionMetadata posMetadata)
    {
        ...
        /*
         * If it is null, it must be already collapsed. This group metadata object should have been set from the search
         * that returned the position metadata object.
         */
    ...
    /**
     * Searches the expandable list adapter for a group position matching the given group ID. The search starts at the
     * given seed position and then alternates between moving up and moving down until 1) we find the right position, or
     * 2) we run out of time, or 3) we have looked at every position
     * 
     * @return Position of the row that matches the given row ID, or {@link AdapterView#INVALID_POSITION} if it can't be
     *         found
    ...
    /**
     * Metadata about an expanded group to help convert from a flat list position to either a) group position for
     * groups, or b) child position for children
     */
    ...
    /**
     * Data type that contains an expandable list position (can refer to either a group or child) and some extra
     * information regarding referred item (such as where to insert into the flat list, etc.)
     */
    static public class PositionMetadata
    {
        ...
        /**
         * Link back to the expanded GroupMetadata for this group. Useful for removing the group from the list of
         * expanded groups inside the connector when we collapse the group, and also as a check to see if the group was
         * expanded or collapsed (this will be null if the group is collapsed since we don't keep that group's metadata)
         */
        public GroupMetadata groupMetadata;

        /**
         * For groups that are collapsed, we use this as the index (in mExpGroupMetadataList) to insert this group when
         * we are expanding this group.
         */
        ...
        static PositionMetadata obtain(int flatListPos, int type, int groupPos, int childPos,
