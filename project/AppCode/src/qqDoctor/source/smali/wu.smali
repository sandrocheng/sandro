.class final Lwu;
.super Landroid/database/ContentObserver;


# instance fields
.field private synthetic a:Lwr;


# direct methods
.method constructor <init>(Lwr;Landroid/os/Handler;)V
    .locals 0

    iput-object p1, p0, Lwu;->a:Lwr;

    invoke-direct {p0, p2}, Landroid/database/ContentObserver;-><init>(Landroid/os/Handler;)V

    return-void
.end method


# virtual methods
.method public final onChange(Z)V
    .locals 7

    const/4 v6, 0x1

    invoke-super {p0, p1}, Landroid/database/ContentObserver;->onChange(Z)V

    if-nez p1, :cond_0

    iget-object v0, p0, Lwu;->a:Lwr;

    invoke-static {v0}, Lwr;->n(Lwr;)Lki;

    move-result-object v0

    const/16 v1, 0xf

    invoke-virtual {v0, v1, v6}, Lki;->a(II)Lcom/tencent/tmsecure/module/aresengine/SmsEntity;

    move-result-object v0

    if-eqz v0, :cond_0

    iget-object v1, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->phonenum:Ljava/lang/String;

    if-nez v1, :cond_1

    :cond_0
    :goto_0
    return-void

    :cond_1
    iget-object v1, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->phonenum:Ljava/lang/String;

    const-string v2, "+86"

    const-string v3, ""

    invoke-virtual {v1, v2, v3}, Ljava/lang/String;->replace(Ljava/lang/CharSequence;Ljava/lang/CharSequence;)Ljava/lang/String;

    move-result-object v1

    iput-object v1, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->phonenum:Ljava/lang/String;

    iget-object v1, p0, Lwu;->a:Lwr;

    invoke-static {v1}, Lwr;->b(Lwr;)Ljd;

    move-result-object v1

    invoke-virtual {v1}, Ljd;->A()I

    move-result v1

    invoke-static {}, Lwr;->n()Z

    iget-object v2, p0, Lwu;->a:Lwr;

    invoke-static {v2}, Lwr;->c(Lwr;)Z

    move-result v2

    if-eqz v2, :cond_2

    invoke-static {}, Ljava/lang/System;->currentTimeMillis()J

    move-result-wide v2

    iget-object v4, p0, Lwu;->a:Lwr;

    invoke-static {v4}, Lwr;->d(Lwr;)J

    move-result-wide v4

    sub-long/2addr v2, v4

    invoke-static {v2, v3}, Ljava/lang/Math;->abs(J)J

    move-result-wide v2

    mul-int/lit8 v1, v1, 0x3c

    mul-int/lit16 v1, v1, 0x3e8

    int-to-long v4, v1

    cmp-long v1, v2, v4

    if-ltz v1, :cond_0

    :cond_2
    iget-object v1, p0, Lwu;->a:Lwr;

    invoke-static {v1, v0}, Lwr;->a(Lwr;Lcom/tencent/tmsecure/module/aresengine/TelephonyEntity;)Z

    move-result v1

    if-eqz v1, :cond_0

    const-string v1, "kfc"

    const-string v2, "sms observered by ContentObserver"

    invoke-static {v1, v2}, Lff;->a(Ljava/lang/String;Ljava/lang/Object;)V

    const-string v1, "kfc"

    new-instance v2, Ljava/lang/StringBuilder;

    const-string v3, "from: "

    invoke-direct {v2, v3}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    iget-object v3, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->phonenum:Ljava/lang/String;

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v2

    const-string v3, ", body: "

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v2

    iget-object v3, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->body:Ljava/lang/String;

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v2

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v2

    invoke-static {v1, v2}, Lff;->a(Ljava/lang/String;Ljava/lang/Object;)V

    iget-object v1, p0, Lwu;->a:Lwr;

    invoke-static {v1}, Lwr;->n(Lwr;)Lki;

    move-result-object v1

    invoke-virtual {v1, v0}, Lki;->remove(Lcom/tencent/tmsecure/module/aresengine/SmsEntity;)Z

    const/4 v1, 0x2

    iget-object v2, p0, Lwu;->a:Lwr;

    invoke-static {v2}, Lwr;->h(Lwr;)I

    move-result v2

    if-eq v1, v2, :cond_0

    iget-object v1, p0, Lwu;->a:Lwr;

    invoke-static {v1}, Lwr;->i(Lwr;)I

    move-result v1

    if-nez v1, :cond_3

    invoke-static {}, Ljava/lang/System;->currentTimeMillis()J

    move-result-wide v1

    iget-object v3, p0, Lwu;->a:Lwr;

    invoke-static {v3}, Lwr;->j(Lwr;)J

    move-result-wide v3

    sub-long/2addr v1, v3

    const-wide/16 v3, 0x3e8

    div-long/2addr v1, v3

    long-to-int v1, v1

    if-lez v1, :cond_3

    iget-object v2, p0, Lwu;->a:Lwr;

    invoke-static {v2}, Lwr;->b(Lwr;)Ljd;

    move-result-object v2

    invoke-virtual {v2, v1}, Ljd;->l(I)V

    :cond_3
    iget-object v1, p0, Lwu;->a:Lwr;

    invoke-static {v1, v6}, Lwr;->a(Lwr;I)I

    iget-object v1, p0, Lwu;->a:Lwr;

    invoke-static {v1}, Lwr;->k(Lwr;)Lwr$b;

    move-result-object v1

    invoke-virtual {v1, v0}, Lwr$b;->a(Lcom/tencent/tmsecure/module/aresengine/SmsEntity;)Lcom/tencent/tmsecure/module/aresengine/SmsEntity;

    move-result-object v0

    if-eqz v0, :cond_0

    iget-object v1, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->body:Ljava/lang/String;

    if-eqz v1, :cond_0

    iget-object v1, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->body:Ljava/lang/String;

    iget-object v2, p0, Lwu;->a:Lwr;

    invoke-static {v2}, Lwr;->l(Lwr;)Ljava/lang/String;

    move-result-object v2

    invoke-virtual {v1, v2}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result v1

    if-nez v1, :cond_4

    iget-object v1, p0, Lwu;->a:Lwr;

    iget-object v2, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->phonenum:Ljava/lang/String;

    invoke-static {v1, v2}, Lwr;->b(Lwr;Ljava/lang/String;)Ljava/lang/String;

    iget-object v1, p0, Lwu;->a:Lwr;

    iget-object v0, v0, Lcom/tencent/tmsecure/module/aresengine/SmsEntity;->body:Ljava/lang/String;

    invoke-static {v1, v0}, Lwr;->c(Lwr;Ljava/lang/String;)Ljava/lang/String;

    iget-object v0, p0, Lwu;->a:Lwr;

    iget-object v1, p0, Lwu;->a:Lwr;

    invoke-static {v1}, Lwr;->m(Lwr;)Ljava/lang/String;

    move-result-object v1

    iget-object v2, p0, Lwu;->a:Lwr;

    invoke-static {v2}, Lwr;->l(Lwr;)Ljava/lang/String;

    move-result-object v2

    invoke-static {v0, v1, v2}, Lwr;->a(Lwr;Ljava/lang/String;Ljava/lang/String;)V

    goto/16 :goto_0

    :cond_4
    const-string v0, "kfc"

    const-string v1, "repeated sms, AresEngine\'s bug xxxxxxx"

    invoke-static {v0, v1}, Lff;->a(Ljava/lang/String;Ljava/lang/Object;)V

    goto/16 :goto_0
.end method
